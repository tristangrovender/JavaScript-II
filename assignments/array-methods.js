...
let fullNames = [];
runners.forEach(function(athletes) {
  fullNames.push(`${athletes.first_name} ${athletes.last_name}`)
});
...
const firstNamesAllCaps = runners.map((athletes) => {
  return athletes.first_name.toUpperCase()
});
...
let runnersLargeSizeShirt = runners.filter((athletes) => {
  return athletes.shirt_size === "L"
});
console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let ticketPriceTotal = runners.reduce((total, athletes) => {
  return total += athletes.donation;
}, 0);
...
// Problem 1
// Log the average donation amount for this data set
averageDonation = runners.reduce((total, athletes) => {
  return total += athletes.donation;
}, 0)

console.log(`The average donation amount is: ${averageDonation/50}`);


// Problem 2
// Make a new array containing only representatives company and first name for promotional use.
nameAndCompany = runners.map((athletes) => {
  return (`${athletes.first_name} ${athletes.company_name}`)
});
...
// Problem 3
// Make a new array containing only participants name and email so that HR can send out thank you letters. They do not need donation amounts etc.
desensitizedInfo = runners.map((athletes) => {
  return (`name:${athletes.first_name} email:${athletes.email}`)
})

console.log(desensitizedInfo);